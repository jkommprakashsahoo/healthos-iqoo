...
export async function getStoredAppointments(userId: string): Promise<StoredAppointment[]> {
  if (!supabase) return [];
  const { data, error } = await supabase
    .from('appointments')
    .select('id, doctor_name, specialty, iso_date, time, location, notes')
    .eq('user_id', userId)
...
export async function saveStoredAppointment(
  appointment: Omit<StoredAppointment, 'id'>,
  userId: string
  ...
  const { data, error } = await supabase
    .from('appointments')
    .insert({ id, user_id: userId, ...appointment })
    .select('id, doctor_name, specialty, iso_date, time, location, notes')
    .single();
  if (error) throw new Error(error.message);
  return data as StoredAppointment;
}